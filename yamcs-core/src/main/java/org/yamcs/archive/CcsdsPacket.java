...
public class CcsdsPacket extends PacketWithTime {
	...
	/**
	 * constructs a packet setting the reception time to the current time
	 * @param packet
	 */
	public CcsdsPacket(byte[] packet) {
		this(TimeEncoding.currentInstant(), packet);			
	}

	public CcsdsPacket(long receptionTime, byte[] packet) {
		super(receptionTime, getInstant(packet), packet);
